...
public class PluginGroup extends PluginRecord {
    ...
    /**
     * Set the ignore status for the group. Should recurse down
     * the entire tree.
     *
    ...
    /**
     * Only applicable for the DIAL top group. Takes the map of deleted INFOs and goes
     * through the group looking for the INFO that has each deleted form ID as a PNAM. If found,
     * then replace that PNAM with the form ID in the PNAM of the deleted INFO. This is the 
     * "suturing", effectively sewing the PNAM sequence back together after the removal of the
     * INFO. The iteration is done for each deleted record to ensure consistency in the event that
     * "chains" of INFOs have been deleted.
     * 
     * @param	List<PluginRecord>	List of deleted INFO records
     * @return                      void
     */
    public void suturePNAMs(Map<Integer, List<PluginRecord>> deletedMap)
    {
    	if (groupType != TOP || !this.groupRecordType.equals("DIAL")) return;
    	if (deletedMap.isEmpty()) return;
		...
		for (PluginRecord dialOrInfo : groupList)
		{
			if (dialOrInfo instanceof PluginGroup) // INFO Group
			{
				...
				// Hackish attempt to ensure that chained topics get PNAMs replaced in the correct order.
				for (PluginRecord delRec : deletedList)
				{
					...
					{
						try { if (rec.changeSubrecord("PNAM", delFormID, delPNAMID)) break; }
    ...
    /**
     * Only applicable for the DIAL top group. Goes through each topic group looking for
     * deleted INFOs. When found, each deleted record is inserted
     * 
     * @return	Map<Integer, List<PluginRecord>>	Lists of deleted INFOs, keyed to topic ID.
     */
    public Map<Integer, List<PluginRecord>> getDeletedINFOMap()
    {
    	...
    	if (groupType != TOP || !this.groupRecordType.equals("DIAL")) return deletedINFOMap;
		List<PluginRecord> groupList = getRecordList();
		for (PluginRecord dialOrInfo : groupList)
		{
			if (dialOrInfo instanceof PluginGroup) // INFO Group
			{
				...
				deletedINFOMap.put(topicID, infoList);
			}			
		}
    ...
    /**
     * Returns a list of all PluginRecords in the PluginGroup. Recurses merrily
     * down the tree until it hits the end of each group branch. No particular order
     * is guaranteed.
     * 
    ...
    /**
     * Returns a list of all deleted PluginRecords in the PluginGroup. Recurses merrily
     * down the tree until it hits the end of each group branch. No particular order
     * is guaranteed.
     * 
     * @return                      The list of deleted PluginRecords
     */
