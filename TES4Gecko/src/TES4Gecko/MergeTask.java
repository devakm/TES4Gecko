...
public class MergeTask extends WorkerTask {
    ...
    /** Whether to use a "floor" form ID */
    private boolean useFloorFormID = false;
    
    /** Floor Form ID [0 if unused] */
    private int floorFormID;
    
    /** Deleted NFO map for suturing. Now a member for greater access. */
    ...
    public void run() {
        ...
        // Find whether the merge should be "managed"; if not, none of the merging inquiry popups
    	// will appear and default behavior will ensue.
    	...
    	if (masterMerge)
    	{
	    	int selection = JOptionPane.showConfirmDialog(parentWindow, 
	                ...
	                "intervention at points within the merge;\notherwise default merge behavior will take place.", 
	                "Manage Merge?", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	    	if (selection == JOptionPane.YES_OPTION) manageMerge = true;
    	}
    	try {
            //
            // Load each plugin
            ...
            // Only for master merge and when the user wishes to manage the merge:
            // get the region exterior cell info from the plugin that is to be merged into
            // the master. Based on that info, go through the master list and the master
            // to be merged and gather the info for the dialog. None of this silly 
            // pseudo-generality here either: the master is plugin[0] 
            // and the to-be-merged plugin is plugin[1]. :)
            if (masterMerge && manageMerge)
            {
            	...
            	if (cellRegionList.size() > 0)
            	{
            		...
            		for (int region : cellRegionList)
            		{
            			...
            			if (region == Plugin.NoRegionAssignedToCell)
            			{
            				// Simply put a dummy entry in regionData and continue.
                    		String[] regionInfoArray = new String[4];
            				regionInfoArray[0] = String.format("%08X", Plugin.NoRegionAssignedToCell);
            			...
            			else
            			{
            				...
            				else if (pluginName.equalsIgnoreCase(plugins[0].getName()))
            					// Must be the master to be merged to.
            				{
            					pluginToSearch = plugins[0];
            				}
        				...
        				if (pluginRec == null) // Most likely cause: different ESM version, like Oblivion.ESM with or w/o SI.
        				{
        			        if (Main.debugMode)
        			        {
        				...
        				if (pluginRec != null) 
        				{
	        				...
	        				catch (Exception ex) // Possible for a region not to have a WNAM.
	        				{
	        			        if (Main.debugMode)
	        			        {
	        				...
	        				// If we've gotten this far, only one more piece of info to get: the worldspace name.
	        				// Unfortunately, we get to repeat a lot of what was done for the region.
	        				// Check for the easy and usual case first. One very important complication is that
	        				// for load order independence, world spaces with zero index may be introduced in 
	        				// other plugins than the indexed plugin. This means that not finding the WS in the 
	        				// indexed plugin is not the end of the search.
	        				int WSIndex = (worldspaceID == -1) ? worldspaceID : worldspaceID >>> 24;
	        				String WSPluginName = "";
	        				if (WSIndex != masterIdx // In a different plugin, so must search for it.
	        					&& WSIndex != -1) // Worldspace was found
	        				{
	                			if (WSIndex == pluginMasterList.size()) // The plugin itself
	                			{
	                				pluginToSearch = plugins[1];
	                				WSPluginName = plugins[1].getName();
	                			}
	                			else
	                			{
	                				WSPluginName = pluginMasterList.get(WSIndex);
	                				int mergedMasterIdx = mergedMasterList.indexOf(WSPluginName);
	                				if (mergedMasterIdx != -1)
	                				{
	                					pluginToSearch = masters[mergedMasterIdx];
	                				}
	                				else if (WSPluginName.equalsIgnoreCase(plugins[0].getName()))
	                					// Must be the master to be merged to.
	                				{
	                					pluginToSearch = plugins[0];
	                				}
	                				else // Something is wrong...
	                				{
	                			        if (Main.debugMode)
	                			        {
	                			            System.out.print("MergeTask: Plugin " + WSPluginName + " for worldspace "
	                			            		+ String.format("%08X", worldspaceID) + " not found in merge list\n");
	                			        }
	                			        continue;
	                				}
	                			}
	        				}
	        				else if (WSIndex == masterIdx) // Look in same place as for the region.
	        				{
	        					WSPluginName = pluginName;
	        				}
	        				if (WSIndex == -1) // Worldspace not found
	        				{
	        					pluginToSearch = new String(); 
	        					// A bit of trickery to bypass the instanceof conditions below. A check of
	        					// WSIndex is still needed in the debug statement.
	        				}
	        				...
	        				if (WSFormInfo == null && WSIndex != -1)
	        				{
	        					if (WSIndex == 0)
	        					{
	        						// A worldspace with index zero can be introduced within any plugin in order for LOD
	        						// to work properly, so if not found within the zero-index plugin, look in the plugin
	        						// to merge and then in the merged master list. Only admit defeat after that.
	        						// First look in the plugin to be merged.
	        						WSFormInfo = plugins[1].getFormMap().get(worldspaceID);
	        						if (WSFormInfo == null) // The master to be merged into.
	        						{
		        						WSFormInfo = plugins[0].getFormMap().get(worldspaceID);
		        						if (WSFormInfo == null) // The master list.
		        						{
		        							for (int j = masters.length; j >= 0; j--)
		        							{
		        								WSFormInfo = masters[j].getFormMap().get(worldspaceID);
	        				...
	        				if (WSFormInfo != null)
	        			        ...
	        			        if (Main.debugMode)
	        			        {
	        			            System.out.print("MergeTask: Worldspace " + String.format("%08X", worldspaceID)
	        			            		       + " not found in plugin " + WSPluginName + "\n");
	        			        }

        				}
        				// Now fill in the region info.
                		String[] regionInfoArray = new String[4];
        				regionInfoArray[0] = String.format("%08X", region);
        				regionInfoArray[1] = pluginName;
        				regionInfoArray[2] = worldspaceName;
        				regionInfoArray[3] = regionName;
        				regionData.add(regionInfoArray);
                    ...
                    // Now parse the result and set ignore accordingly.
                    String[] resultArray = retStr.split(RegionCellDialog.Separator);
                    // For ALL, do nothing.
                    if (resultArray[0].equals(RegionCellDialog.All)) ;;
                    if (resultArray[0].equals(RegionCellDialog.None)) plugins[1].ignoreAllExteriorCells();
                    if (resultArray[0].equals(RegionCellDialog.CancelMerge)) throw new InterruptedException("Merge canceled");
                    if (resultArray[0].equals(RegionCellDialog.Some))
                    {
                    	ArrayList<Integer> regionIDs = new ArrayList<Integer>();
                    	for (int j = 1; j < resultArray.length; j++) // Skipping first array member
                    	{
                    		int regionID = Integer.parseInt(resultArray[j], 16);
                    		regionIDs.add(regionID);
                    	}
                    	plugins[1].ignoreAllExteriorCellsExcept(regionIDs);                    	
                    }
                    if (resultArray[0].equals(RegionCellDialog.Except))
                    {
                    	ArrayList<Integer> regionIDs = new ArrayList<Integer>();
                    	for (int j = 1; j < resultArray.length; j++) // Skipping first array member
                    	{
                    	...
                    	plugins[1].ignoreAllExteriorCells(regionIDs);                    	
                    }
            	}
            } // End of mind-numbingly long "exclude exterior cells" block
            
            // Now check to see if a "floor" form ID is to be used. Cancelling or negating the 
            // initial popup does the merge the usual way; cancelling or merging the succeeding
            // form ID popup cancels the merge. Also only when the user wishes to manage the merge.
            if (masterMerge && manageMerge)
            {
	            int selection2 = JOptionPane.showConfirmDialog(parentWindow, 
	                    ...
	                    "Select Starting Form ID", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	            if (selection2 == JOptionPane.YES_OPTION)
	            {
	            	useFloorFormID = true;
	            	floorFormID = getStartFormID(highestFormID(plugins[0]));
	            	if (floorFormID == -1)
	            	{
	            		useFloorFormID = false;
	                    throw new InterruptedException("Merge canceled");
	            	}
	            	floorFormID++;
	            }
	            else useFloorFormID = false;
            ...
            deletedINFOMap = new HashMap<Integer, List<PluginRecord>>();
        ...
        //
        // All done, but zero out the form lists for all plugins and masters involved to reduce loitering.
        //
        if (mergedPlugin != null)
        {
	        mergedPlugin.resetFormList();
	        mergedPlugin.resetFormMap();
        }
        for (i=0; i<plugins.length; i++) plugins[i].resetFormList();
        for (i=0; i<masters.length; i++) masters[i].resetFormList();
        for (i=0; i<plugins.length; i++) plugins[i].resetFormMap();
        for (i=0; i<masters.length; i++) masters[i].resetFormMap();
    ...
    private void mapPluginRecords(Plugin plugin) throws DataFormatException, IOException, PluginException {
        ...
        for (FormInfo info : formList) {
            ...
            // If record is ignored, it is neither mapped nor used.
            ...
            if (masterReference) {
                ...
                //
                // Check for a Form ID collision. If we are merging a plugin into
                ...
                if (mergedInfo != null) {
                    if (masterMerge && masterID < masterCount) {
                        ...
                        if (!recordType.equals("WRLD") && !recordType.equals("CELL") && !recordType.equals("DIAL")) {
                            ...
                            {
                            	// As above, ignore both records. Hopefully the auto-removal of ignored records will do the rest.
                                mergedRecord = (PluginRecord)mergedInfo.getSource();
                                mergedRecord.setIgnore(true);
                            	record.setIgnore(true);
                            }
                    ...
                    else if (masterMerge && useFloorFormID && !plugin.isMaster())// Where the base form ID stuff will go.
                    {
                    	formID = floorFormID++;
                    }
                    else
    ...
    private void mergeGroup(PluginGroup mergedGroup, PluginGroup group)
        ...
        while (lit.hasNext()) {
            ...
            if (record instanceof PluginGroup) {
                ...
                // beginning of the merged record list.
                //
                // IMPORTANT UPDATE: Precise order DOES matter, and that order is:
                // Persistent => VWD => Temporary
                if (!subgroupMerged) 
                {
                    ...
                    else if (subgroupType == PluginGroup.CELL_DISTANT)
                    {
                    	if (mergedRecordList.size() > 0)
                    	{
                    		int insertIndex = 0;
                    		PluginGroup firstGroup = (PluginGroup)mergedRecordList.get(0);
                    		if (firstGroup.getGroupType() == PluginGroup.CELL_PERSISTENT)
                    			insertIndex = 1; // VWD after persistent.
                    ...
                    else if (subgroupType == PluginGroup.CELL_PERSISTENT) // Always first.
                    {
                        mergedRecordList.add(0, record);
                        record.setParent(mergedGroup);
                    }
                ...
                int index = mergedRecordList.indexOf(record);
            	// Putting this code here may eliminate the need for suturePNAMs() totally.
                // On second thought, it appears that new topic groups are implanted as a whole
                // w/o using addTopic(). This means the earlier method is still necessary.
            	if (recordType.equals("INFO") && !record.isDeleted() && !deletedINFOMap.isEmpty()
            			&& deletedINFOMap.containsKey(group.getGroupParentID()))
            	{
            		// We need to check if the PNAM for the altered INFO is in the deleted list
            		// and change it if that is true.
    				List<PluginRecord> deletedList = deletedINFOMap.get(group.getGroupParentID());
    				Collections.reverse(deletedList);
    				// Same hack (hell, same code) as in suturePNAMs().
    				for (PluginRecord delRec : deletedList)
    				{
    		    		...
    		    		try { if (record.changeSubrecord("PNAM", delFormID, delPNAMID)) continue; }
    		    		catch (Exception ex) { continue; }
    				}
    				// Since the list may be used multiple times, undo the hack.
    				Collections.reverse(deletedList);
            	}
     ...
     * @exception   PluginException         The plugin is not valid
     */
    private void mergeDialogTopic(PluginRecord mergedRecord, PluginRecord record) 
    throws DataFormatException, IOException, PluginException
    {
        boolean modifiedMergedList = false;
        ...
        // 
        // Since DIALs are not stored in masters, we have to do the grunt work here.
        // Luckily, there's not too much. Just set the non-QSTI fields (EDID & FULL)
        // to either the merged or plugin value depending on the shouldDeleteLastConflict()
        // value, and then add the new QSTI records to the megred record.
        if (masterMerge || !pluginInfo.shouldDeleteLastConflict()) // Use plugin values; always overwrite for master merge
        {
     ...
     * - Then the plugin is scanned to add any INFO records that the plugin itself deletes that
     ...
     */
 	private int getStartFormID(int baseID)
 	{
 		...
 		String inputID = (String)JOptionPane.showInputDialog(
                 parentWindow, "<html>Please enter the starting form ID <i>in hex</i>"
